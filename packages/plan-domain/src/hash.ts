...
function sha256PureSync(message: string): string {
...
function sha256Hex(canonical: string): string {
  return sha256PureSync(canonical);
}
 ...
 * Synchronous pure SHA-256 keeps computeInputHash sync in browser bundles
 * (crypto.subtle is async; node:crypto is unavailable in the Vite client bundle).
...
export function computeInputHash(inputs: Record<string, JsonValue>): string {
  return `sha256:${sha256Hex(canonicalize(inputs))}`;
}
