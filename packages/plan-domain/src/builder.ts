...
function requirementApplies(rule: RequirementRule, enabledObjectives: Set<string>): boolean {
  switch (rule.type) {
    ...
    case 'NEVER':
      return false;
    case 'ANY_OBJECTIVE_ENABLED':
      return enabledObjectives.size > 0;
    case 'OBJECTIVE_ENABLED':
      return rule.objectiveIds.some((objectiveId) => enabledObjectives.has(objectiveId));
  }
}
...
export function buildPlanVersionRecord(
  ...
  sectionDefinitions: PlanSectionDefinition[],
  enabledObjectiveIds: string[] = [],
): PlanBuildResult {
  ...
  const statuses = evaluatePlanSectionStatuses(sectionDefinitions, normalizedObjectives);
